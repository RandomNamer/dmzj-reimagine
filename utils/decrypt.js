
const crypto = require("crypto");
const { COMIC_V1_PRIVATE_KEY, COMIC_V2_PRIVATE_KEY } = require("./safestore");
const { time } = require("console");

/**
 * Calculated by the salt and Android package cert on app 3.9.1, considered stable.
 */
const CORETOKEN_SUFFIX_V3 = "C0:39:91:5A:7F:45:67:9D:8B:3E:3F:2D:AF:54:93:41"
const CORE_TOKEN_STRING_PREFIX = "com.dmzj.manhua63:60:C8:3B:75:31:3F:35:EC:41:1D:85:60:63:EB:25"
const CORE_TOKEN_STRING_SUFFIX = "+bYV5TaOBivUHM"


function decryptComicV1Response(text){
    return decryptBlocksWithKey(text, COMIC_V1_PRIVATE_KEY, 128)
}

/**
 * For comic v2 APIs.
 */
function decryptComicV2Response(text){
    return decryptBlocksWithKey(text, COMIC_V2_PRIVATE_KEY, 256)
}

function decryptBlocksWithKey(text, key, blockLength){
    let pemKey  = crypto.createPrivateKey({
        key: Buffer.from(key, 'base64'),
        format: 'der',
        type: 'pkcs8',
    })
    var buf = Buffer.from(text, 'base64')
    let textLen = buf.byteLength;
    var loc = 0;
    var decryptedBufs = [];
    while(loc < textLen){
        if( (textLen - loc) > blockLength){
            try {
                decryptedBufs.push(
                    crypto.privateDecrypt({
                        key: pemKey,
                        padding: crypto.constants.RSA_PKCS1_PADDING,
                    }, buf.slice(loc, loc + blockLength))
                )
            } catch (error) {
                console.log(error)
            }
        }
        else {
            try {
                decryptedBufs.push(
                    crypto.privateDecrypt({
                        key: pemKey,
                        padding: crypto.constants.RSA_PKCS1_PADDING,
                    }, buf.slice(loc, textLen))
                )
            } catch (error) {
                console.log(error)
            }
        }
        loc += blockLength;
    }
    return Buffer.concat(decryptedBufs)
}

/**
 * extracted from Android app 3.9.1 in libdmzj.so.
 */
function getCoreTokenV3() {
    const timestamp = Math.floor(Date.now() / 1000);
    const tokenString = `${CORE_TOKEN_STRING_PREFIX}${timestamp}${CORE_TOKEN_STRING_SUFFIX}`
    const digest = crypto.createHash('md5').update(tokenString).digest('hex')
    const newHash = Buffer.from(digest, 'hex').toString('hex').match(/.{1,2}/g).join(":").toUpperCase();
    return `${timestamp}|${newHash}`
}

module.exports = {decryptComicV1Response, decryptComicV2Response, getCoreTokenV3}

// console.log(getCoreTokenV3())
// let s = decryptBlocksWithDefaultKey("")
// console.log(s)